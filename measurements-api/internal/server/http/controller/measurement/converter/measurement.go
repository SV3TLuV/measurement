...
import (
	...
	model2 "measurements-api/internal/server/http/controller/measurement/model"
	model6 "measurements-api/internal/server/http/model"
)

func ToMeasurementViewFromService(measurement *model.Measurement) *model2.MeasurementView {
	if measurement == nil {
		return nil
	}

	return &model2.MeasurementView{
		ID:                   measurement.ID,
...
func ToMeasurementViewsFromService(measurements []*model.Measurement) []*model2.MeasurementView {
	views := make([]*model2.MeasurementView, 0, len(measurements))
	for i := 0; i < len(measurements); i++ {
		views = append(views, ToMeasurementViewFromService(measurements[i]))
	}
	return views
}

func ToGetMeasurementsParamsFromQuery(query *model2.FetchMeasurementListQuery) *model3.GetMeasurementsParams {
	if query == nil {
		return nil
	}

	return &model3.GetMeasurementsParams{
...
func ToMeasurementPagedListViewFromService(
	pagedList *model.PagedList[model.Measurement]) *model6.PagedListView[model2.MeasurementView] {
	if pagedList == nil {
		return nil
	}

	return &model6.PagedListView[model2.MeasurementView]{
		Page:     pagedList.Page,
		PageSize: pagedList.PageSize,
		Total:    pagedList.Total,
		Items:    ToMeasurementViewsFromService(pagedList.Items),
	}
}

func ToGetMeasurementParamsFromExportQuery(query model2.ExportMeasurementListQuery) *model3.GetMeasurementsParams {
	return &model3.GetMeasurementsParams{
		ObjectID: query.ObjectID,
		Period:   query.Period,
		Start:    query.Start,
...
func ToContentTypeFromExportFormat(format model.ExportFormat) string {
	switch format {
	case model.Xlsx:
